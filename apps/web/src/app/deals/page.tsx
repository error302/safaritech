      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-12 overflow-hidden rounded-2xl bg-gradient-to-r from-red/20 via-accent/20 to-electric/20 p-8 md:p-12">
          <div className="relative z-10">
            ...
            <Link href="#flash" className="btn btn-primary">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {flashSales.map((product) => (
              <Link
                ...
                className="group relative rounded-xl border border-border bg-card overflow-hidden transition-all hover:border-electric/50"
              >
                ...
                <div className="aspect-square bg-surface flex items-center justify-center text-muted">
                  Product Image
                </div>
                <div className="p-4">
        ...
        <section>
          <div className="mb-6 flex items-center gap-2">
            <Tag className="h-8 w-8 text-electric" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {deals.map((product) => (
              <Link
                key={product.id}
                href={`/product/${product.id}`}
                className="group relative rounded-xl border border-border bg-card overflow-hidden transition-all hover:border-electric/50 hover:shadow-lg hover:shadow-electric/10"
              >
                <div className="absolute left-2 top-2 z-10 rounded-full bg-green px-2 py-1 text-xs font-bold text-white">
                  {product.discount}% OFF
                </div>
                <div className="absolute right-2 top-2 z-10 flex items-center gap-1 rounded-full bg-charcoal/80 px-2 py-1 text-xs text-muted">
                  <Clock className="h-3 w-3" />
                  {product.endsIn}
                </div>
                <div className="aspect-square bg-surface flex items-center justify-center text-muted">
