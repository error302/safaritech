...
import { Smartphone, Laptop, Gamepad2, Headphones, Search, SlidersHorizontal } from "lucide-react";
  ...
  const FilterPanel = () => (
    <aside className="w-full md:w-64 shrink-0 space-y-5">
          ...
          {brands.map((brand) => (
            <label key={brand} className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                className="rounded border-safariborder accent-neon"
              />
      ...
      <button
        ...
        className="w-full border border-safariborder text-gray-500 hover:border-neon hover:text-neon text-sm py-2.5 rounded-xl transition-all font-semibold"
      >
        Reset Filters
      </button>
    </aside>
        ...
        <div className="max-w-7xl mx-auto px-4 py-5">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-0.5">{filtered.length} products found</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                className="md:hidden flex items-center gap-2 border border-safariborder text-gray-500 text-sm px-4 py-2 rounded-xl hover:border-neon transition-all"
              ...
              <select
                ...
                className="bg-safarigray border border-safariborder rounded-xl px-4 py-2 text-sm text-gray-300 focus:outline-none focus:border-neon/50"
              >
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto pb-1 scrollbar-none">
            <button
              ...
              className={`shrink-0 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                selectedCat === "all" ? "bg-neon text-black" : "bg-safaridark border border-safariborder text-gray-400 hover:border-neon/30 hover:text-neon"
              }`}
            >
              All
            </button>
              ...
              return (
                <button
                  ...
                  className={`shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                    selectedCat === cat.id ? "bg-neon text-black" : "bg-safaridark border border-safariborder text-gray-400 hover:border-neon/30 hover:text-neon"
                  }`}
                >
