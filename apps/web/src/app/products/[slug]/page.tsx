...
import { ShieldCheck, RotateCcw, Truck, Smartphone, ChevronRight } from "lucide-react";
        ...
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
          <div className="md:sticky md:top-24 self-start">
            <div className="relative aspect-square rounded-3xl overflow-hidden bg-safarigray border border-safariborder mb-4">
              <Image src={product.images?.[0] || "/placeholder.jpg"} alt={product.name} fill className="object-cover" priority />
              {discount && (
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              {product.stock > 0 ? (
                <span className="inline-flex items-center gap-1.5 bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-semibold px-3 py-1.5 rounded-full font-display">
                  <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                  In Stock - Ships today
                </span>
              ) : (
                <span className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold px-3 py-1.5 rounded-full">
                  Out of Stock
                </span>
              )}
            </div>
            ...
            <div>
              <p className="text-xs text-gray-600 font-semibold tracking-wider uppercase mb-1.5">{product.category?.name}</p>
              <h1 className="font-display font-black text-3xl md:text-4xl text-white leading-tight">{product.name}</h1>
            </div>
            ...
            <div className="flex flex-col gap-3 pt-2">
              <button
                className="w-full bg-neon hover:bg-neon-dim text-black font-display font-bold text-base py-4 rounded-2xl transition-all active:scale-95 disabled:opacity-50"
              ...
              <button className="w-full bg-green-600 hover:bg-green-500 text-white font-display font-bold text-base py-4 rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2">
                <Smartphone className="w-5 h-5" /> Buy Now with M-Pesa
              </button>
            </div>
              ...
              {[
                ...
                return (
                  <div key={item.text} className="bg-safarigray border border-safariborder rounded-xl p-3 text-center">
                    <Icon className="w-5 h-5 text-neon/60 mx-auto mb-1.5" />
        ...
        {related.length > 0 && (
          <section className="mt-20">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link href={"/shop?cat=" + product.category?.slug} className="text-sm text-gray-500 hover:text-neon transition-colors">
                View all
              </Link>
            </div>
